...
def resize_image(image, max_width=1920, max_height=1080):
    ...
    resized_img = cv.resize(image, (w, h), interpolation=cv.INTER_LANCZOS4)
    return resized_img


# Function to apply blur effect while keeping selected areas sharp
def apply_blur_with_boxes(image_path, blur_strength=50):
    global img, img_copy, boxes, drawing, ix, iy, temp_box
...
def estimate_measurement(image_path, ref_obj_width_real):
    ...
    input_image = apply_blur_with_boxes(image_path)
